...
def _input_dt(method, obj_func, c_v, month, egr_spotpy=None):
...
if __name__ == "__main__":
    ...
    calib_valid = 'valid'
    month='dec'
    simluation_res = 'D:\Gaby\Dt\Calib\simular\dec\dream\\aic\\'
    ind_simul = [1000, 1500]

    for m in ['dream']: #'demcz', 'dream', 'fscabc','mle']: #
        for obj_func in ['aic', 'mic']: #, 'nse', 'rmse', 'aic',  'mic'
            if calib_valid == 'calib':
                tipo_proc, guardar, guar_sim, egr_spotpy = _input_dt(method=m, obj_func=obj_func, c_v=calib_valid, egr_spotpy=egr_spotpy, month=month)
