...
class Modelo(object):
    ...
    def _validar(símismo, bd, var, corresp_vars, tipo_proc, guardar, lg, paralelo,
        ...
        if n_sim is None:
            ...
            wt = np.asarray([(p - np.min(top_prob)) / np.ptp(top_prob) for p in top_prob])
        ...
        if not n_sim:
            ...
            matrs_simul = {
                vr: {'top_res': top_res, 'all_res': all_res, 'wt_res': wt_res, 'mu_res': mu_res, 'mdn_res': mdn_res,
                     'top_prob': top_prob} for vr in
                l_vars}
